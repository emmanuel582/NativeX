...
app.post('/webhook', async (req, res) => {
  ...
  const dateTimeMatch = messageText && messageText.match(/(\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4})\s+(\d{1,2}:\d{2}(?:\s*[ap]m)?)/i) || messageText.match(/(\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4})\s*(\d{1,2}:\d{2}[ap]m)/i);
  if (dateTimeMatch && global.lastDriveUpload) {
    ...
    const dateTimeStr = `${parseDate(date)} ${time.replace(/([ap]m)$/i, ' $1')}`;
    const scheduleDate = new Date(dateTimeStr);
    if (isNaN(scheduleDate) || scheduleDate <= new Date()) {
      await sendWhatsAppMessage(from, "Invalid or past schedule time. Please reply in DD/MM/YYYY HH:mm am/pm format and use a future time.");
      return res.status(200).send('EVENT_RECEIVED');
    }
    const { fileName, cloudinaryUrl, folderName, uniqueId, captions } = global.lastDriveUpload;
    let platforms = [];
    ...
    const hashtags = extractHashtags(captions.facebook || captions.tiktok || '');
    ...
    await sendWhatsAppMessage(from, `✅ 10X has scheduled '${fileName}' for ${scheduleDate.toLocaleString()} on ${platforms.join(', ')}.`);
    global.lastDriveUpload = null;
    return res.status(200).send('EVENT_RECEIVED');
  }

  // 2. List scheduled posts for a date
  const listMatch = messageText && messageText.match(/what do we have to post on (\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4})/i);
  if (listMatch) {
    function parseDate(input) {
      const match = input.match(/(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})/);
      if (!match) return null;
  ...
  if (aboutMatch) {
    ...
    if (post) {
      ...
      if (post.cloudinaryUrl) reply += `Cloudinary URL: ${post.cloudinaryUrl}\n`;
...
app.post("/distribute", async (req, res) => {
  ...
  try {
    ...
    if (platforms.some((p) => ["facebook", "instagram", "linkedin"].includes(p))) {
    ...
    if (scheduleTime) {
      ...
      cron.schedule(cronTime, async () => {
        ...
        await ScheduledPost.findByIdAndUpdate(scheduledDoc._id, { status: "completed" });
...
app.post("/process-transcript", async (req, res) => {
  try {
    ...
    const command = `whisper "${videoPath}" --language English --model base --output_format txt --output_dir "${path.join(
      __dirname,
      "uploads"
    )}"`;
    ...
    exec(command, async (error, stdout, stderr) => {
      if (error) {
        ...
        if (
          ...
          return res.status(500).json({
            success: false,
            error:
              "Whisper is not installed. Please install Whisper first: pip install openai-whisper",
          });
        }
      ...
      try {
        ...
        if (fs.existsSync(transcriptPath)) {
          ...
          if (geminiService) {
            try {
              content = await geminiService.generateAllPlatformContent(
                transcript.trim()
              );
            } catch (geminiError) {
              console.error(
        ...
        console.error(
          "Error reading transcript or generating content:",
          readError
        );
        res.status(500).json({
          success: false,
          error: "Failed to process transcript: " + readError.message,
        });
      }
    });
  } catch (error) {
...
async function sendWhatsAppMessage(to, message) {
    try {
        // Ignore 'to', always send to your target number via local bot
        await axios.post('http://localhost:3000/send', { text: message });
        console.log('✅ Message sent to local WhatsApp bot.');
    } catch (error) {
        console.error('❌ Error sending to local WhatsApp bot:', error.message);
    }
}
...
async function initializeFolderMap() {
  ...
  console.log('Initialized Google Drive polling. Will only notify for new files from now on.');
}
...
async function handleNewDriveVideo(file, folder) {
  try {
    ...
    let captions = {};
    try {
      const transcriptRes = await axios.post(`http://localhost:${port}/process-transcript`, { filename: file.name });
      if (transcriptRes.data.success) {
        captions = transcriptRes.data.content;
        console.log(`✅ Captions generated for all platforms.`);
      } else {
        console.warn(`⚠️ Transcript processing failed, using folder name as caption.`);
      }
    } catch (e) {
      captions = { tiktok: folder.name, instagram: folder.name };
      console.warn(`⚠️ Transcript/caption generation error, using folder name as fallback.`);
    }
...
async function watchFolderAndSubfolders() {
  ...
  for (const folder of subfolders) {
    ...
    for (const file of newOnes) {
      if (file.mimeType !== 'application/vnd.google-apps.folder' && file.mimeType.startsWith('video/')) {
        ...
        console.log(`🆕 New folder added to folder: ${folder.name} → ${file.name}`);
      }
    }
